...
public class Map {
	...
	public void addFigureOnElement(Coordinates c, Figure f, PlayerState p) {
	...
	public Coordinates findThisFigure(FigureType figuretype, PlayerState player) {
		try {
			for(int x = 0; x < this.getNumberOfColumns(); x++) {
				for(int y = 0; y < this.getNumberOfRows(); y++) {
					for(Figure figure :  this.elements.get(new Coordinates(x,y)).getOnField()) {
						if(figure.getFigureType().equals(figuretype) && figure.getFigureOwnerPlayerId().equals(player.getUniquePlayerID())) {
							return new Coordinates(x,y); 
						}
					}
				}
			}
			if(figuretype.equals(FigureType.AVATAR) || figuretype.equals(FigureType.CASTLE)) {
			...
			Logger.getAnonymousLogger().log(Level.SEVERE, e.getMessage()); // EDIT!
