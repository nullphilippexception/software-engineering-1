...
public class MapElement {
	...
	public boolean containsFigure(FigureType figureType, PlayerState player) {
		for(Figure f : onField) {
			if(f.getFigureType().equals(figureType) && f.getFigureOwnerPlayerId().equals(player.getUniquePlayerID())) return true; 
		}
		return false;
	}
	
	// removes a figure from this mapelement, returns true if figure was actually present on this element
	public boolean removeFigure(FigureType figuretype, PlayerState player) {
		try {
			if(!this.containsFigure(figuretype, player)) { 
				throw new IllegalArgumentException("Tried to remove " + figuretype + " which is not on this element");
			}
			else {
		...
		catch(IllegalArgumentException e) { // checked exception
